...
def question_1h_sanity_check():
    ...
    print("Running Sanity Check for Question 1g: question_1g_sanity_check")
    print ("-"*80)
    x_conv = torch.tensor([[0.1, 0.2, 0.4], [0.3, 0.4, 0.1]])  # 2X3
    D_in= 3
    #vocab = VocabEntry()
    highway = Highway(D_in)
    x_highway = highway(x_conv)

    print("Sanity Check x_highway.shape="+str(x_highway.shape))
    print("x_highway="+str(x_highway))
    print("-"*80)
