...
class CharDecoder(nn.Module):
    def __init__(self, hidden_size, char_embedding_size=50, target_vocab=None):
        ...
        self.charDecoder = nn.LSTM(char_embedding_size, hidden_size, 1) #input_size, hidden_size, num_layers=2
    ...
    def forward(self, input, dec_hidden=None):
        ...
        input_char_embedings = self.decoderCharEmb(input) #先找到字符的embeding # (length, batch, char_embed_size)
        hiddens, dec_hidden = self.charDecoder(input_char_embedings, dec_hidden)
        score = self.char_output_projection(hiddens)  # 将维度从hidden_dim扩到v_char # (len, batch, Ｖ_char)
    ...
    def train_forward(self, char_sequence, dec_hidden=None):
        ...
        score, (h, c) = self.forward(char_sequence[:-1], dec_hidden)  # score.shape =   (length, batch, self.vocab_char_size)
        ...
        # take input : (N, C), target (N) ,此处的N 相当于　length*batch, C为Ｖ_char 即字符库的大小
    ...
    def decode_greedy(self, initialStates, device, max_length=21):
        ...
        b = initialStates[0].shape[1]  # batch_size
